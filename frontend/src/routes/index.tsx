...
const totalNodes = graphDonut.reduce((a, b) => a + b.value, 0);
...
function Index() {
  ...
  const timeLabel = (s: string) => s.replace(/^Hoje/, tt("Hoje")).replace(/^Ontem/, tt("Ontem"));
  return (
    <AppLayout
      title={t("dash.title")}
    ...
    >
            ...
            <Card key={s.label}>
              <CardContent className="p-4">
                <div className="flex items-start justify-between">
                  <p className="text-sm font-medium text-muted-foreground">{tt(s.label)}</p>
                  <div className={`grid h-9 w-9 place-items-center rounded-lg ${statTint[s.type]}`}>
                    <Icon className="h-4 w-4" />
                  </div>
                </div>
                <p className="mt-2 text-2xl font-bold tracking-tight">{s.value.toLocaleString(locale)}</p>
                <p className="mt-1 text-xs text-success">{s.change.replace("esta semana", tt("thisWeek"))}</p>
              </CardContent>
            </Card>
          );
        })}
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-1">
              ...
              {recentSearches.map((s) => (
                <div key={s.query} className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 rounded-lg px-2 py-2 hover:bg-secondary/50">
                  <span className="flex min-w-0 items-center gap-2 text-sm">
                    <Search className="h-3.5 w-3.5 shrink-0 text-muted-foreground" />
                    <span className="truncate">{s.query}</span>
                    <Badge variant="outline" className={`${artifactTypeColors[s.tag]} shrink-0`}>{s.tag}</Badge>
                  </span>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-2">
              <Select>
                ...
                <SelectContent>
                  {stories.map((s) => (
                    <SelectItem key={s.id} value={s.id}>{s.id} — {s.title}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
              <Button asChild>
            ...
            <div className="space-y-1">
              ...
              {recentAnalyses.map((a) => (
                <div key={a.story} className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 rounded-lg px-2 py-2 hover:bg-secondary/50">
                  <span className="flex min-w-0 items-center gap-2 text-sm">
                    <span className="truncate">{a.story}</span>
                    <Badge variant="outline" className={`${riskBadgeColor[a.risk]} shrink-0`}>{tt(a.risk)}</Badge>
                  </span>
            ...
            <div className="flex items-center gap-4">
              <div className="relative h-36 w-36 shrink-0">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={graphDonut} dataKey="value" innerRadius={48} outerRadius={68} paddingAngle={2} stroke="none">
                      {graphDonut.map((d) => (
              ...
              <div className="min-w-0 flex-1 space-y-1.5">
                {graphDonut.map((d) => (
                  <div key={d.name} className="flex items-center gap-2 text-sm">
                    ...
                    <span className="truncate">{tt(d.name)}</span>
                    <span className="ml-auto text-muted-foreground">{d.value.toLocaleString(locale)}</span>
                  </div>
                ))}
              </div>
            </div>
            <Link to="/graph" className="mt-4 flex items-center gap-1 text-sm font-medium text-primary">
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-3">
        <Card>
          ...
          <CardContent className="space-y-1">
            {recentActivity.map((a) => {
              ...
              return (
                <div key={a.id} className="flex gap-3 rounded-lg px-2 py-2.5 hover:bg-secondary/50">
                  <Icon className={`mt-0.5 h-4 w-4 shrink-0 ${meta.cls}`} />
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium">{a.title}</p>
                    <p className="truncate text-xs text-muted-foreground">{a.desc}</p>
                  </div>
                  <span className="shrink-0 text-xs text-muted-foreground">{timeLabel(a.time)}</span>
                </div>
              );
            })}
            <Link to="/artifacts" className="flex items-center gap-1 pt-2 text-sm font-medium text-primary">
        ...
        <Card>
          ...
          <CardContent className="space-y-1">
            {topBugs.map((b, i) => (
              <div key={b.id} className="grid grid-cols-[auto_auto_minmax(0,1fr)_auto_auto] items-center gap-3 rounded-lg px-2 py-2.5 hover:bg-secondary/50">
                <span className="w-3 text-sm text-muted-foreground">{i + 1}</span>
                <span className="font-mono text-xs text-muted-foreground">{b.id}</span>
                <span className="truncate text-sm">{b.title}</span>
                <Badge variant="outline" className={severityColor[b.severity]}>{tt(b.severity)}</Badge>
